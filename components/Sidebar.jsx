...
const Sidebar = ({
    ...
    (term) => {
      ...
      collections.forEach((collection) => {
        collection.data.data.forEach((chapter) => {
          if (
            chapter.attributes.titulo.toLowerCase().includes(term.toLowerCase())
          ) {
            results.push({
  ...
  const handleSubChapterClick = useCallback(
    (collectionId, subChapterId) => {
      router.push(
        `/collection/${collectionId}/#subcapitulo_${subChapterId}`,
        undefined,
        { shallow: true }
      );
      closeSidebar();
  ...
  const toggleSubChapters = useCallback(
    (chapterId) => {
